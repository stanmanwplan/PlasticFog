...
schedule matched the one derived on paper before the machinery existed, and the
answer did not move.
...
Every number the assert script checks was derived by hand in advance, from a
trace of the unfarmed campaign with each round's duals and the perturbed
vectors printed at full precision. The optimum is the same as the unfarmed
lane's, because farming buys columns per round and nothing else: an objective
that moved would be a defect rather than a result. The campaign takes one round
fewer, because a column that the unfarmed run reaches at a later round enters
the restricted master earlier.
