...
row per execution, and the assertions check four things together: that the
expected number of rows exists per campaign, that the objective is identical on
every run, that the interval campaign's runs start at least `intervalSeconds`
apart measured from the rows' own timestamps, and that the runs which carried
warm start are marked as having reused the cached artifacts. That last check is
the standing guard — without it, "warm start carried" would be a claim about
intent rather than a measurement, and a campaign that silently recompiled
everything each time would look identical.
